...
def SortHand(self) -> None:
    ...
    self.hand.sort(key=lambda card: (order_map[card.rank], card.suit or ""))
```
```python
...
def GetActionTrace(self) -> Sequence[Tuple[int, str]]:
    return copy.copy(self.action_trace)
```
```python
...
def SortUnique(self, seq: Sequence[str]) -> Sequence[str]:
    ...
    tmp.sort(key=functools.cmp_to_key(lambda a, b: -1 if CompareActions(a, b) else 1))
    return tmp
```
```python
...
def Run(self) -> None:
    ...
    while not self.judger.IsGameOver(self.players):
        ...
        if len(action) == 0:
            ...
            action_as_string = ActionToString(action)
        is_legal = False
        ...
        if not is_legal:
            ...
            action = ParseActionStringToCards(self.players[current_player_id], fallback_action_str)
