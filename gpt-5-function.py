...
def NewPlayer(id: int) -> Player:
    ...
    def RemoveCards(self, cards: PlayAction) -> None:
        for played_card in cards:
            ...
            for i in range(0, len(self.hand)):
                if Player.CardsEqual(self, self.hand[i], played_card):
                    del self.hand[i]
                    removed = True
                    break
            if not removed:
    ...
    def SelectAction(self, state: Dict) -> PlayAction:
        ...
        if chosen_str == "pass":
            ...
            return self.ParseActionStringToCards(self, chosen_str)
...
def NewRound(players: Sequence[Player], judger: 'Judger') -> 'Round':
    ...
    def GetAllPlayedCards(self) -> Sequence[str]:
        ...
        ranks_list.sort(key=lambda r: order_map.get(r, len(order_map)))
        return ranks_list
...
def NewActionGenerator() -> ActionGenerator:
    ...
    def SubCounts(self, a: Dict[str, int], b: Dict[str, int]) -> Dict[str, int]:
        n: Dict[str, int] = self.CloneCounts(a)
        for k, v in b.items():
            n[k] = n.get(k, 0) - v
            if n[k] < 0:
                n[k] = 0
    ...
    def SortRanks(self, ranks: Sequence[str]) -> Sequence[str]:
        ...
        ranks_list.sort(key=lambda rank: self.RANK_TO_VAL[rank])
        return ranks_list
    ...
    def ListPairRanksFromCounts(self, cnt: Dict[str, int]) -> Sequence[str]:
        ...
        out = self.SortRanks(out)
        return out
    def CombinationsByIndex(self, A: Sequence[Any], k: int) -> Sequence[Sequence[int]]:
    ...
    def IdentifyPatternFromString(self, action_str: str) -> Dict[str, Any]:
        ...
        if len(action_str) == 4:
            if self.ContainsCount(cnt, 3) and self.ContainsCount(cnt, 1) and (self.NumberOfKeys(cnt) == 2):
                ...
                if trio_rank != single_rank:
                    ...
                    info["main_value"] = self.RANK_TO_VAL[trio_rank]  # type: ignore[index]
                    info["core_count"] = 1
                    return info
        if len(action_str) == 5:
            ...
            if self.ContainsCount(cnt, 4):
                ...
                if len(rem_ranks) == 2:
                    if not (("B" in rem_ranks) and ("R" in rem_ranks)):
                        ...
                        info["main_value"] = self.RANK_TO_VAL[four_rank]  # type: ignore[index]
                        return info
        if len(action_str) == 8:
            if self.ContainsCount(cnt, 4):
                four_rank = self.GetRankWithCount(cnt, 4)
        ...
        if core.get("success"):
            core_ranks: List[str] = core["core_ranks"]  # type: ignore[assignment]
            k = len(core_ranks)
            top = core_ranks[k - 1]
            remain = self.SubCounts(cnt, core["core_use"])  # type: ignore[arg-type]
    ...
    def SortUnique(self, seq: Sequence[str]) -> Sequence[str]:
        ...
        class _Key:
            def __init__(self, v: str, outer: "ActionGenerator"):
                self.v = v
                self.outer = outer

            def __lt__(self, other: "._Key") -> bool:
                return self.outer.CompareActions(self.v, other.v)

        tmp.sort(key=lambda s: _Key(s, self))
        return tmp
    ...
    def FindSamePatternStronger(self, hand_cards, last_info: Dict[str, Any]) -> Sequence[str]:
        ...
        if kind == "solo":
            ...
            for s in candidates:
                ...
                if (info.get("kind") == "trio_single") and (info.get("core_count") == last_info["core_count"]) and (info.get("main_value") > last_info["main_value"]):
                    out.append(s)
        elif kind == "trio_pair":
            candidates = self.FindTrioWithPair(hand_cards)
            for s in candidates:
                info = self.IdentifyPatternFromString(s)
                if (info.get("kind") == "trio_pair") and (info.get("core_count") == last_info["core_count"]) and (info.get("main_value") > last_info["main_value"]):
                    out.append(s)
        elif kind == "straight":
            candidates = self.FindStraights(hand_cards)
            for s in candidates:
                info = self.IdentifyPatternFromString(s)
                if (info.get("kind") == "straight") and (info.get("length") == last_info["length"]) and (info.get("main_value") > last_info["main_value"]):
                    out.append(s)
        elif kind == "pair_chain":
            candidates = self.FindPairChains(hand_cards)
            for s in candidates:
                info = self.IdentifyPatternFromString(s)
                if (info.get("kind") == "pair_chain") and (info.get("pair_len") == last_info["pair_len"]) and (info.get("main_value") > last_info["main_value"]):
                    out.append(s)
        elif kind == "airplane":
            candidates = self.FindAirplanes(hand_cards)
            for s in candidates:
                info = self.IdentifyPatternFromString(s)
                if (info.get("kind") == "airplane") and (info.get("trio_len") == last_info["trio_len"]) and (info.get("main_value") > last_info["main_value"]):
                    out.append(s)
        elif kind == "airplane_single":
            candidates = self.FindAirplanesWithAttachments(hand_cards)
            for s in candidates:
                info = self.IdentifyPatternFromString(s)
                if (info.get("kind") == "airplane_single") and (info.get("trio_len") == last_info["trio_len"]) and (info.get("main_value") > last_info["main_value"]):
                    out.append(s)
        elif kind == "airplane_pair":
            candidates = self.FindAirplanesWithAttachments(hand_cards)
            for s in candidates:
                info = self.IdentifyPatternFromString(s)
                if (info.get("kind") == "airplane_pair") and (info.get("trio_len") == last_info["trio_len"]) and (info.get("main_value") > last_info["main_value"]):
                    out.append(s)
        elif kind == "four_two_single":
            candidates = self.FindFourWithTwo(hand_cards)
            for s in candidates:
                info = self.IdentifyPatternFromString(s)
                if (info.get("kind") == "four_two_single") and (info.get("main_value") > last_info["main_value"]):
                    out.append(s)
        elif kind == "four_two_pair":
            candidates = self.FindFourWithTwo(hand_cards)
            for s in candidates:
                info = self.IdentifyPatternFromString(s)
                if (info.get("kind") == "four_two_pair") and (info.get("main_value") > last_info["main_value"]):
                    out.append(s)
        elif kind == "bomb":
            pass
        else:
...
class Game:
    def GetOthersHandAsString(self, exclude_player_id: int) -> str:
        ...
        for ch in chars:
            result = result + ch
        return result
    def BuildState(self, current_player_id: int, landlord_id: int, seen_cards, legal_actions: Sequence[str]) -> Dict[str, Any]:
    ...
    def Run(self) -> None:
        ...
        while not self.judger.IsGameOver(self.players):
            ...
            if not is_legal:
                ...
                action = self.players[current_player_id].ParseActionStringToCards(self.players[current_player_id], fallback_action_str)
