...
def ParseActionStringToCards(player: Player, action_str: str) -> PlayAction:
    result = []
    if action_str == "pass":
        return result
    rank_to_indices = {}
...
def GetHandAsString(self) -> str:
    ...
    bucket = {rank: 0 for rank in rank_order}
...
def RankAfter(self, r: str) -> Optional[str]:
    idx = self.RANK_TO_VAL[r]
    if idx >= len(self.RANK_ORDER) - 1: return None
    return self.RANK_ORDER[idx + 1]
...
def AllSameCount(self, cnt: Dict[str, int], value: int) -> bool:
    if self.NumberOfKeys(cnt) == 0: return False
    for r, c in cnt.items():
...
def SortRanks(self, ranks: Sequence[str]) -> Sequence[str]:
    return sorted(ranks, key=lambda rank: self.RANK_TO_VAL[rank])
    return sorted(ranks, key=lambda rank: self.RANK_TO_VAL[rank])
...
def IsConsecutive(self, ranks_sorted: Sequence[str]) -> bool:
    if len(ranks_sorted) <= 1: return True
...
def SortUnique(self, seq: Sequence[str]) -> Sequence[str]:
    ...
    for s in seq:
        if s not in seen:
            seen.add(s)
            tmp.append(s)
    tmp.sort(key=lambda x: self.RANK_TO_VAL[x])
    return tmp
...
def Run(self) -> None:
    ...
    while not self.judger.IsGameOver(self.players):
        ...
        state = self.BuildState(current_player_id, landlord_id, self.seen_cards, legal_actions)
...
# The total prompt token usage is:  17310952
# The total completion token usage is:  11237