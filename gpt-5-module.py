...
class Player:
    ...
    def GetHandAsString(self) -> str:
        rank_order: List[str] = ["3","4","5","6","7","8","9","T","J","Q","K","A","2","B","R"]
...
class ActionGenerator:
    ...
    def CountRanks(self, hand_cards: Sequence[Any]) -> Dict[str, int]:
        m: Dict[str, int] = {}
        for c in hand_cards:
    ...
    def SortRanks(self, ranks: Sequence[str]) -> List[str]:
        return sorted(list(ranks), key=lambda rank: self.RANK_TO_VAL[rank])
    ...
    def CombinationsByIndex(self, A: Sequence[Any], k: int) -> List[List[int]]:
            ...
            if self.AllSameCount(cnt, 2) and (self.NumberOfKeys(cnt) == 1):
                ...
                for k, v in cnt.items():
                    if v > 0:
                        ...
                        break
                info["kind"] = "pair"
                info["main_value"] = self.RANK_TO_VAL[r] if r is not None else -1
                return info
        if len(action_str) == 3:
        ...
        if len(action_str) == 4:
            if self.ContainsCount(cnt, 3) and self.ContainsCount(cnt, 1) and (self.NumberOfKeys(cnt) == 2):
                ...
                if trio_rank != single_rank:
                    ...
                    info["main_value"] = self.RANK_TO_VAL[trio_rank] if trio_rank is not None else -1
                    info["core_count"] = 1
                    return info
        if len(action_str) == 5:
            ...
            if self.ContainsCount(cnt, 4):
                ...
                if len(rem_ranks) == 2:
                    if not (("B" in rem_ranks) and ("R" in rem_ranks)):
                        ...
                        info["main_value"] = self.RANK_TO_VAL[four_rank] if four_rank is not None else -1
                        return info
        if len(action_str) == 8:
            if self.ContainsCount(cnt, 4):
                four_rank = self.GetRankWithCount(cnt, 4)
    ...
    def SortUnique(self, seq: Sequence[str]) -> List[str]:
        ...
        for s in seq:
            if s not in seen:
                seen.add(s)
                tmp.append(s)
        # Insertion sort using CompareActions
        for i in range(1, len(tmp)):
            key = tmp[i]
            j = i - 1
            while j >= 0 and self.CompareActions(key, tmp[j]):
                tmp[j + 1] = tmp[j]
                j -= 1
            tmp[j + 1] = key
        return tmp
    ...
    def CompareActions(self, a: str, b: str) -> bool:
        ...
        if (ia.get("kind") != "invalid") and (ib.get("kind") != "invalid"):
            if ia.get("main_value") != ib.get("main_value"):
                return ia.get("main_value") < ib.get("main_value")
            return self.LexRankLess(a, b)
        else:
            return self.LexRankLess(a, b)

    def FindSolos(self, hand_cards: Sequence[Any]) -> List[str]:
        result: List[str] = []
    ...
    def FindPairs(self, hand_cards: Sequence[Any]) -> List[str]:
        result: List[str] = []
        counts = self.CountRanks(hand_cards)
        for r, c in counts.items():
    ...
    def FindTrios(self, hand_cards: Sequence[Any]) -> List[str]:
        result: List[str] = []
        counts = self.CountRanks(hand_cards)
        for r, c in counts.items():
    ...
    def FindTrioWithSingle(self, hand_cards: Sequence[Any]) -> List[str]:
        result: List[str] = []
        counts = self.CountRanks(hand_cards)
        for r, c in counts.items():
            if c >= 3 and r != "B" and r != "R":
    ...
    def FindTrioWithPair(self, hand_cards: Sequence[Any]) -> List[str]:
        result: List[str] = []
        counts = self.CountRanks(hand_cards)
        for r, c in counts.items():
            if c >= 3 and r != "B" and r != "R":
    ...
    def FindStraights(self, hand_cards: Sequence[Any]) -> List[str]:
        result: List[str] = []
        counts = self.CountRanks(hand_cards)
        elig: List[str] = []
        for r, c in counts.items():
    ...
    def FindPairChains(self, hand_cards: Sequence[Any]) -> List[str]:
        result: List[str] = []
        counts = self.CountRanks(hand_cards)
        elig: List[str] = []
        for r, c in counts.items():
    ...
    def FindAirplanes(self, hand_cards: Sequence[Any]) -> List[str]:
        result: List[str] = []
        counts = self.CountRanks(hand_cards)
        elig: List[str] = []
        for r, c in counts.items():
    ...
    def FindAirplanesWithAttachments(self, hand_cards: Sequence[Any]) -> List[str]:
        result: List[str] = []
        counts = self.CountRanks(hand_cards)
    ...
    def FindFourWithTwo(self, hand_cards: Sequence[Any]) -> List[str]:
        result: List[str] = []
        counts = self.CountRanks(hand_cards)
        for r, c in counts.items():
            if c >= 4:
    ...
    def FindBombs(self, hand_cards: Sequence[Any]) -> List[str]:
        result: List[str] = []
        counts = self.CountRanks(hand_cards)
        for r, c in counts.items():
    ...
    def FilterHigherBombs(self, hand_cards: Sequence[Any], last_bomb_value: int) -> List[str]:
    ...
    def HasRocket(self, hand_cards: Sequence[Any]) -> bool:
        counts = self.CountRanks(hand_cards)
        return (counts.get("B", 0) >= 1) and (counts.get("R", 0) >= 1)

    def FindSamePatternStronger(self, hand_cards: Sequence[Any], last_info: Dict[str, Any]) -> List[str]:
        out: List[str] = []
        ...
        if kind == "solo":
            ...
            for s in candidates:
                ...
                if (info.get("kind") == "trio_single") and (info.get("core_count") == last_info.get("core_count")) and (info.get("main_value") > last_info.get("main_value")):
                    out.append(s)
        elif kind == "trio_pair":
            candidates = self.FindTrioWithPair(hand_cards)
            for s in candidates:
                info = self.IdentifyPatternFromString(s)
                if (info.get("kind") == "trio_pair") and (info.get("core_count") == last_info.get("core_count")) and (info.get("main_value") > last_info.get("main_value")):
                    out.append(s)
        elif kind == "straight":
            candidates = self.FindStraights(hand_cards)
            for s in candidates:
                info = self.IdentifyPatternFromString(s)
                if (info.get("kind") == "straight") and (info.get("length") == last_info.get("length")) and (info.get("main_value") > last_info.get("main_value")):
                    out.append(s)
        elif kind == "pair_chain":
            candidates = self.FindPairChains(hand_cards)
            for s in candidates:
                info = self.IdentifyPatternFromString(s)
                if (info.get("kind") == "pair_chain") and (info.get("pair_len") == last_info.get("pair_len")) and (info.get("main_value") > last_info.get("main_value")):
                    out.append(s)
        elif kind == "airplane":
            candidates = self.FindAirplanes(hand_cards)
            for s in candidates:
                info = self.IdentifyPatternFromString(s)
                if (info.get("kind") == "airplane") and (info.get("trio_len") == last_info.get("trio_len")) and (info.get("main_value") > last_info.get("main_value")):
                    out.append(s)
        elif kind == "airplane_single":
            candidates = self.FindAirplanesWithAttachments(hand_cards)
            for s in candidates:
                info = self.IdentifyPatternFromString(s)
                if (info.get("kind") == "airplane_single") and (info.get("trio_len") == last_info.get("trio_len")) and (info.get("main_value") > last_info.get("main_value")):
                    out.append(s)
        elif kind == "airplane_pair":
            candidates = self.FindAirplanesWithAttachments(hand_cards)
            for s in candidates:
                info = self.IdentifyPatternFromString(s)
                if (info.get("kind") == "airplane_pair") and (info.get("trio_len") == last_info.get("trio_len")) and (info.get("main_value") > last_info.get("main_value")):
                    out.append(s)
        elif kind == "four_two_single":
            candidates = self.FindFourWithTwo(hand_cards)
            for s in candidates:
                info = self.IdentifyPatternFromString(s)
                if (info.get("kind") == "four_two_single") and (info.get("main_value") > last_info.get("main_value")):
                    out.append(s)
        elif kind == "four_two_pair":
            candidates = self.FindFourWithTwo(hand_cards)
            for s in candidates:
                info = self.IdentifyPatternFromString(s)
                if (info.get("kind") == "four_two_pair") and (info.get("main_value") > last_info.get("main_value")):
                    out.append(s)
        elif kind == "bomb":
            pass
        else:
    ...
    def GenerateAllPatterns(self, hand_cards: Sequence[Any]) -> List[str]:
        result: List[str] = []
    ...
    def GetLegalActions(self, player: "Player", round_context: "Round") -> List[str]:
        ...
        if last_info.get("kind") == "invalid":
            all_patterns = self.GenerateAllPatterns(hand_cards)
            for s in all_patterns:
                actions.append(s)
            return self.SortUnique(actions)
        if last_info.get("kind") == "rocket":
            return self.SortUnique(actions)
        stronger = self.FindSamePatternStronger(hand_cards, last_info)
        for s in stronger:
            actions.append(s)
        if last_info.get("kind") != "bomb":
            bombs = self.FindBombs(hand_cards)
            for b in bombs:
                actions.append(b)
        else:
            higher_bombs = self.FilterHigherBombs(hand_cards, last_info.get("main_value"))
...
class Game:
    ...
    def BuildState(self, current_player_id: int, landlord_id: int, seen_cards: Sequence[Any], legal_actions: Sequence[str]) -> Dict[str, Any]:
