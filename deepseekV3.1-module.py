...
def DetermineLandlord(self, players: Sequence[Player]) -> int:
    ...
    for player in players:
        ...
        score = EvaluateHandHeuristic(hand_str)
        if score > best_score:
            best_score = score
...
def ParseActionStringToCards(player, action_str: str) -> PlayAction:
    ...
    for ch in action_str:
        if ch in rank_to_indices and rank_to_indices[ch]:
...
class Player:
    ...
    def RemoveCards(self, cards: PlayAction) -> None:
        for played_card in cards:
            ...
            for i in range(len(self.hand) - 1, -1, -1):
                if CardsEqual(self.hand[i], played_card):
                    del self.hand[i]
                    removed = True
                    break
            if not removed:
    ...
    def SelectAction(self, state: Dict[str, Any]) -> PlayAction:
        ...
        if chosen_str == "pass":
            ...
            return ParseActionStringToCards(self, chosen_str)
...
def RecordAction(self, player_id: int, action: PlayAction) -> None:
    if len(action) == 0:
        ...
        action_str = ActionToString(action)
...
def GetAllPlayedCards(self) -> Sequence[str]:
    ...
    rank_order = self.judger.rank_order
    ranks_list.sort(key=lambda r: rank_order.index(r))
    return ranks_list
```
```python
from __future__ import annotations
...
def NewActionGenerator() -> ActionGenerator:
    ...
    ag.RANK_TO_VAL = {}
    for i, rank in enumerate(ag.RANK_ORDER):
        ag.RANK_TO_VAL[rank] = i
...
def SortRanks(self, ranks: Sequence[str]) -> List[str]:
    return sorted(ranks, key=lambda r: self.RANK_TO_VAL[r])
...
def StringFromCounts(self, cnt: Dict[str, int]) -> str:
    s = ""
    ranks_sorted = self.SortRanks(list(cnt.keys()))
    for r in ranks_sorted:
...
def IdentifyPatternFromString(self, action_str: str) -> Dict[str, Any]:
    ...
    if len(action_str) == 5:
        if self.ContainsCount(cnt, 3) and self.ContainsCount(cnt, 2) and self.NumberOfKeys(cnt) == 2:
            ...
            if trio_rank != pair_rank and pair_rank not in ["B", "R"]:
                info["kind"] = "trio_pair"
                info["main_value"] = self.RANK_TO_VAL[trio_rank]
                info["core_count"] = 1
                return info
...
def SortUnique(self, seq: Sequence[str]) -> List[str]:
    ...
    tmp.sort(key=lambda x: (len(x), self.IdentifyPatternFromString(x)["main_value"] if self.IdentifyPatternFromString(x)["kind"] != "invalid" else 0, x))
    return tmp

def FindSolos(self, hand_cards) -> List[str]:
    result = []
...
def FindPairs(self, hand_cards) -> List[str]:
    result = []
    counts = self.CountR
```python
from __future__ import annotations
